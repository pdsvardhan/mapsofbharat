...
test.describe("feat-social-export", () => {
  ...
  test("the card draws the Survey-of-India boundary note (item 847)", async ({ page }) => {
    // Same fillText-interception style as the estimate-footnote test: the boundary
    // self-certification line is drawn inside the card canvas, so assert it via the
    // captured fills rather than pixel-reading text.
    test.setTimeout(120_000);
    await page.addInitScript(() => {
      const w = window as unknown as { __fills: string[] };
      w.__fills = [];
