// Social export compositor — ROUND-1 DESIGN VARIANT COPY (item 762).
// Working copy of lib/social-export.ts, refactored so every block placement
// reads from a LAYOUT preset. Six presets follow the original renderer's
// drawing code verbatim wherever composition is not the thing being varied:
// palette, fonts, accent treatment, stroke weights and copy are FROZEN.
//
// What changed vs the shipped file:
//   1. fitProjection now also reports the rect India actually occupies, so the
//      four empty regions the owner complained about (NW beyond Kashmir, the
//      Tibet band above the north-east, the Arabian Sea, the Bay of Bengal) can
//      be addressed as real coordinates instead of guessed.
//   2. A void allocator hands those regions out to blocks top-down / bottom-up,
//      so island insets and content blocks share a sea without colliding.
//   3. Every block (headline, anchor, rank tables, legend, note, insets) is
//      placed from the preset, and every placed rect is fed back into the
//      on-map label collision set so state cards cannot draw text under a panel.
//
// Nothing here is committed; this file exists only under /tmp/mob-design/round1.
...
export type SocialCardSpec = {
  ...
  accentWords?: number[];
  /** Round-1 addition: which composition to draw. Default "v0" = shipped layout. */
  layout?: LayoutId;
};
...
export async function renderSocialCard(spec: SocialCardSpec): Promise<HTMLCanvasElement> {
  ...
  const L = LAYOUTS[spec.layout ?? "v0"];
  ...
  const N = Math.max(3, Math.min(10, TS.rows));
  ...
  if (!dense) {
    ...
    for (let pass = 0; pass < 5; pass++)
      for (const l of flung) {
        ...
        for (const r of reserved) if (hit(boxOf(l), r)) l.y = r.y - (TIERS[l.tier].h - 18) - 10;
        l.y = Math.max(safe.y + 24, Math.min(safe.y + safe.h - 18, l.y));
      }
