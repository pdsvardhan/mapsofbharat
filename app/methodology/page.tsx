      ...
      <dl className="mt-4 space-y-3">
        {(
          [
            ...
            ["bivariate", "Paired maps", "Two metrics on one geography, coloured from a 3\u00d73 matrix: one across, one down. It answers the question two side-by-side maps make you answer by eye \u2014 where do these go together, and where do they come apart \u2014 in one place, with no flicking back and forth. Three classes per axis and not four: nine fills is already at the limit of what a reader can hold without a key, and sixteen stops being nameable. Both sides must be RATES. A total shaded by colour carries the same area bias that proportional circles exist to remove, and a pair map has only colour to work with \u2014 there is no second channel left to escape into \u2014 so a total on either axis is refused, and the map says so rather than quietly drawing it. Both sides must also cover the same ground: the pair holds only above the coverage floor a metric-to-metric comparison already uses, because a pair is only as good as the districts both sides reach. The bands are set over the shared districts alone, so a region the pair does not both cover cannot stretch them, and over the districts on screen alone — drill into a state with fewer than three of them and there is nothing to cut three bands from, so the map says so and stays on the single-metric ramp rather than drawing a matrix nothing is painted from. Each axis is banded INDEPENDENTLY, by the same rule that classes a single-metric map: three bands by rank where the numbers spread, and where they do not — 445 districts report exactly 0% Buddhist — the value everyone shares gets its own band at the bottom, so “none” is never painted as the middle of three. The two boundaries on each axis are printed beside the key. This is the hardest map here to read, so the matrix key is drawn wherever the matrix is — and nowhere else. The paint and the key answer to one condition, so a nine-cell key can never stand over a map that was drawn from a single ramp: where the pair is refused the key stands down with it and the reason takes its place. The table view stays the equivalent for anyone the colours fail."],
            ["value-by-alpha", "Faded by population", "A rate is already measured per person, so circles would invent a distortion that normalising had removed. But the shaded map still gives every district the weight of its acreage, and India\u2019s districts are not the same size or anything like as evenly peopled: 82% of the country\u2019s surface sits in the lowest population-density band while only 51% of Indians do. Where a map\u2019s colour is not where its people are, regions are faded by how many people they hold, so the picture reads closer to the country than to its acreage. The fade never reaches invisibility \u2014 carrying few people is a reason to be quieter, never a reason to be unreadable, and every region stays hoverable, readable and correctable. Fading does cost contrast between neighbouring bands, so the legend carries a second key while it is on: the same colours down a column, each row at the opacity a district of that population is drawn with, so a faded fill can be decoded rather than guessed at. And because a faded fill is a paler fill, a region with NO figure is no longer marked by tone at all \u2014 it is hatched. A texture is the one thing an opacity cannot imitate, so \u201cwe have no number here\u201d stays distinct from \u201ca high number, faintly drawn\u201d at every level of fade. It is applied only where measured to be warranted, and the legend says so and why whenever it fires. It is never used on a count, which has circles instead, nor on the as-reported 2011 view, whose vintage has no density series to weigh with."],
          ] as [string, string, string][]
        ).map(([id, name, body]) => (
          <div key={id} id={id} className="scroll-mt-24 border border-border px-4 py-4" style={{ background: "var(--panel)" }}>
            <dt className="text-[13px] font-semibold text-bright">{name}</dt>
